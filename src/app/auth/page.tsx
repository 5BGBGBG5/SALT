...
export default function AuthPage() {
  ...
  // Validate @inecta.com email
  const validateEmail = (email: string): boolean => {
    return email.toLowerCase().endsWith('@inecta.com');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateEmail(email)) {
      setError('Only @inecta.com email addresses are allowed');
      setLoading(false);
      return;
    }
        ...
        <div className="max-w-md w-full space-y-8">
          <div>
            ...
            <p className="mt-2 text-center text-sm text-gray-600">
              Enter your @inecta.com email address
            </p>
          </div>
          <form className="mt-8 space-y-6" onSubmit={handleForgotPassword}>
            <div>
              ...
              <input
                ...
                placeholder="your.name@inecta.com"
              />
            </div>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      >
        ...
        >
          <h2 className="mt-6 text-center text-3xl font-extrabold bg-gradient-to-r from-white via-teal-200 to-emerald-200 bg-clip-text text-transparent">
            {isLogin ? 'Sign in to your account' : 'Create your account'}
          </h2>
          <p className="mt-2 text-center text-sm text-gray-300">
            SALT — inecta
            <br />
            <span className="text-xs text-gray-400">(@inecta.com emails only)</span>
          </p>
        </motion.div>
        <motion.form 
          initial={{ opacity: 0, y: 10 }}
